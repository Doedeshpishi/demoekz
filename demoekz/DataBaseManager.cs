    ...
    public class DatabaseManager
    {
        ...
        public List<string> GetAllStatuses()
        {
            ...
            using (var conn = new NpgsqlConnection(connectionString))
            {
                ...
                {
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT StatusName FROM Statuses";
        ...
        public List<string> GetAllPriorities()
        {
            ...
            using (var conn = new NpgsqlConnection(connectionString))
            {
                ...
                {
                    cmd.Connection = conn;
                    cmd.CommandText = "SELECT PriorityName FROM Priorities";
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            priorities.Add(reader["PriorityName"].ToString());
                        }
                    }
                }
            }
        ...
        public List<Task> GetTasks(string statusFilter = null, string assigneeFilter = null, string priorityFilter = null)
        {
            ...
            using (var conn = new NpgsqlConnection(connectionString))
            {
                ...
                using (var cmd = new NpgsqlCommand())
                {
                    ...
                    {
                        cmd.CommandText += " WHERE s.StatusName = @StatusFilter";
                    ...
                    if (!string.IsNullOrEmpty(assigneeFilter))
                    {
                        if (!string.IsNullOrEmpty(statusFilter))
                        {
                            cmd.CommandText += " AND a.AssigneeName = @AssigneeFilter";
                        }
                        else
                        {
                            cmd.CommandText += " WHERE a.AssigneeName = @AssigneeFilter";
                        }
                        cmd.Parameters.AddWithValue("AssigneeFilter", assigneeFilter);
                    }

                    if (!string.IsNullOrEmpty(priorityFilter))
                    {
                        if (!string.IsNullOrEmpty(statusFilter) || !string.IsNullOrEmpty(assigneeFilter))
                        {
                            cmd.CommandText += " AND pr.PriorityName = @PriorityFilter";
                        }
                        else
                        {
                            cmd.CommandText += " WHERE pr.PriorityName = @PriorityFilter";
                        }
                        cmd.Parameters.AddWithValue("PriorityFilter", priorityFilter);
                    }
